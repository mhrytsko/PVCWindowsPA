		...
		data() {
			return {
				menuInfo: {
					...
					supportForm: 'management-window-profile',
					goBack: {name: 'home'},
				},
...
<template>
	<v-container>
		<v-responsive class="align-left text-left fill-height">
			<v-row>
				<v-col>
					<m-data-table :table-config="tableConfig" title="Perfies de janelas" @row-click="handleClick">
						<template v-slot:item.image="{item}">
							<v-img :src="getImage(item.raw.imageId)" alt="Profile photo" height="64" />
						</template>
					</m-data-table>
				</v-col>
			</v-row>
